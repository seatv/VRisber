...
class VRSBSProcessor:
    ...
    def __init__(self, config_path: str = "config.json") -> None:
        ...
        self.commands = []  # Track commands for commands-only mode
    ...
    # -------------------------------------------------------------------------
    # Command tracking for commands-only mode
    # -------------------------------------------------------------------------
    def _add_command(self, cmd: List[str], description: str = "") -> None:
        """Add a command to the tracking list for commands-only mode."""
        self.commands.append({
            "cmd": cmd,
            "description": description
        })

    def _write_powershell_script(self, output_path: str) -> None:
        """Write collected commands to a PowerShell script."""
        ps1_path = Path(output_path).with_suffix('.ps1')
        
        with open(ps1_path, 'w', encoding='utf-8') as f:
            f.write("# Vrisber Processing Commands\n")
            f.write("# Generated by Vrisber --commands-only mode\n")
            ...
            for i, cmd_info in enumerate(self.commands, 1):
                ...
                if desc:
                    f.write(f"# Step {i}: {desc}\n")
                else:
                    f.write(f"# Step {i}\n")
                
                # Convert command to PowerShell syntax
                # Use & to call executables and properly escape arguments
                ps_cmd_parts = []
                for part in cmd:
                    ...
                    if ' ' in part or '"' in part or "'" in part or any(c in part for c in ['&', '|', '<', '>', ';', '(', ')']):
                        ...
                        ps_cmd_parts.append(f"'{escaped}'")
                    else:
                        ps_cmd_parts.append(part)
                
                # Format as PowerShell command with & operator
                ps_cmd = f"& {ps_cmd_parts[0]} {' '.join(ps_cmd_parts[1:])}"
                f.write(f"{ps_cmd}\n\n")
            
            f.write("Write-Host 'All commands completed successfully.'\n")
        
        self.logger.info(f"PowerShell script written to: {ps1_path}")
    ...
    def split_sbs_video(
        ...
        self._add_command(cmd_left, "Split left eye stream")
        
        # In commands-only mode, don't execute
        if self.config.get("processing", {}).get("commands_only", False):
            self.logger.info("[Commands-only mode] Would split left eye")
        else:
        ...
        self._add_command(cmd_right, "Split right eye stream")
        
        # In commands-only mode, don't execute
        if self.config.get("processing", {}).get("commands_only", False):
            self.logger.info("[Commands-only mode] Would split right eye")
        else:
    ...
    def extract_audio(self, input_path: str, output_path: str) -> bool:
        ...
        self._add_command(cmd, "Extract audio stream")
        
        # In commands-only mode, don't execute
        if self.config.get("processing", {}).get("commands_only", False):
            self.logger.info("[Commands-only mode] Would extract audio")
            return True
    ...
    def _process_single_eye(
        ...
        # Track command
        self._add_command(cmd, f"Process {eye.lower()} eye stream (sequential)")
        
        # In commands-only mode, don't execute
        if self.config.get("processing", {}).get("commands_only", False):
            self.logger.info("[Commands-only mode] Would process %s eye", eye)
            return True
    ...
    def process_both_eyes(
        ...
        # Track commands
        self._add_command(left_cmd, "Process left eye stream (parallel)")
        self._add_command(right_cmd, "Process right eye stream (parallel)")
        ...
        # In commands-only mode, don't execute
        if self.config.get("processing", {}).get("commands_only", False):
            self.logger.info("[Commands-only mode] Would process both eyes in parallel")
            return True, True
    ...
    def process_file(self, input_path: str, output_path: Optional[str] = None) -> bool:
        ...
        # Reset commands list for this file
        self.commands = []
        ...
        try:
            ...
            # Step 5: Write PowerShell script if in commands-only mode
            if self.config.get("processing", {}).get("commands_only", False):
                self._write_powershell_script(str(output_p))
    ...
    def mux_sbs_video(
        ...
        # Track command
        self._add_command(cmd, "Final SBS mux with audio and encoding")
        
        # In commands-only mode, don't execute
        if self.config.get("processing", {}).get("commands_only", False):
            self.logger.info("[Commands-only mode] Would run final mux/encode")
            return True
